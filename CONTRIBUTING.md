...
Thank you for considering making contributions to Rebus and related repositories!

Rebus uses [Tendermint’s coding repo](https://github.com/tendermint/coding) for overall information on repository
...
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the Rebus team.
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the Rebus repo, to ensure a
    ...
    8. Please make sure to run `make format` before every commit - the easiest way to do this is having your editor run
       ...
       . There are CI tests built into the Rebus repository and all PR’s will require that these tests pass before
...
When proposing an architecture decision for Rebus, please create
...
Now `origin` refers to my fork and `upstream` refers to the Rebus version. So I can `git push -u origin master` to
update my fork, and make pull requests to Rebus from there. Of course, replace `tharsis` with your git handle.
...
with [gogoproto](https://github.com/gogo/protobuf) to generate code for use in Rebus.
...
Rebus uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Rebus repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Rebus utilizes [semantic versioning](https://semver.org/).
...
**Note**: tharsis’s Rebus team currently cuts releases on a need to have basis. We will announce a more
