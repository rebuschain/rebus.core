...
// NewRootCmd creates a new root command for rebusd. It is called once in the
// main function.
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithKeyringOptions(rebushd.Option()).
		WithViper(EnvPrefix)
	...
	rootCmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			serverCtx.Config.Consensus.TimeoutCommit = time.Millisecond * time.Duration(2000)
...
func (a appCreator) appExport(
	...
	if height != -1 {
		rebusApp = app.NewRebus(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)

		if err := rebusApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		rebusApp = app.NewRebus(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)
	}

	return rebusApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
