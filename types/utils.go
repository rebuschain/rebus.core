...
const (
	// MainnetChainID defines the Rebus EIP155 chain ID for mainnet
	MainnetChainID = "reb_1111"
	// TestnetChainID defines the Rebus EIP155 chain ID for testnet
	TestnetChainID = "reb_3333"
)

// IsMainnet returns true if the chain-id has the Rebus mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Rebus testnet EIP155 chain prefix.
...
// GetRebusAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `rebus`).
// The function fails if the provided bech32 address is invalid.
func GetRebusAddressFromBech32(address string) (sdk.AccAddress, error) {
